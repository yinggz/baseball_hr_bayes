...
model {
  ...
  beta_age ~ normal(0, 1);  // weakly informative prior, scale based on age range
  beta_ab ~ normal(0, 0.1);  // weakly informative prior, scale based on at bats range
  beta_velocity ~ normal(0, 1);  // weakly informative prior, scale based on exit velocity range
  beta_angle ~ normal(0, 1);  // weakly informative prior, scale based on launch angle range
  sigma ~ cauchy(0, 5);  // weakly informative prior for standard deviation
...
generated quantities {
  ...
  real p_positive_effect_age = step(beta_age);  // probability that age has a positive effect
  real p_positive_effect_ab = step(beta_ab);  // probability that at bats has a positive effect
  real p_positive_effect_velocity = step(beta_velocity);  // probability that exit velocity has a positive effect
  real p_positive_effect_angle = step(beta_angle);  // probability that launch angle has a positive effect
